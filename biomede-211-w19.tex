...
\begin{itemize}
	...
	\subitem In Figure 2.19: v_1 + (-v_2) + (-v_3) + v_4 + (-v_5) = 0
	\subitem v_1 + v_4 = v_2 +v_3 + v_5
	\subitem Ohm's law can then be applied
	\subitem v_1 - iR_1 - iR_2 + v_4 - \frac{1}{C}\int_{t_0}^{t} i(\tau) d\tau
...
If we look at node x and arbitrarily set i_1 as enterring the node from the left, i_2 as exiting the node down the branch containing the resistor, and i_3 as enterring the node from the right, then:
